...
Function 提取字串符中所有數字(字符串)
    ...
    rExp.Pattern = "\d+"           //設置過濾模式
    rExp.[Global] = True           //設置全局可用性
    Dim Num, Nums                  //定義變量
    Set Nums = rExp.Execute(字符串)//執行搜索  
    For Each Num In Nums           //遍歷匹配集合
        shuzi = shuzi & Num
    next
    Set rExp = Nothing
    提取字串符中所有數字 = shuzi
...
Function 得到字符串中字母的數量(字符串)
    ...
    Dim regEx, Matches, shuliang       //定義變量
    Set regEx = New RegExp             //建立正則表達式
    regEx.Pattern = "([a-z]{1})"       //設置過濾模式
    regEx.IgnoreCase = true            //設置是否區分字符大小寫
    regEx.[Global] = True              //設置全局可用性
    Set Matches = regEx.Execute(字符串)//執行搜索   
